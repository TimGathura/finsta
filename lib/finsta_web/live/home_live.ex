...
defmodule FinstaWeb.HomeLive do
  ...
  def render(assigns) do                                        #verified
    ~H"""
  ...
  @impl true
  def mount(_params, _session, socket) do                       #verified
  ...
  @impl true
  def handle_event("validate", _params, socket) do            #verified
    {:noreply, socket}
  end
  ...
  def handle_event("save-post", %{"post" => post_params}, socket) do
    ...
    |> case do
      {:ok, post} ->
        ...
        IO.inspect(socket.assigns.flash, label: "Flash message after success")
        {:noreply, socket}
  ...
  def consume_files(socket) do                                                #verified
